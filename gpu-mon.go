...
func main() {
	...
	fmt.Printf("\n기준 온도 : %v°C, 기본 클럭 : %vMHz, 현재 클럭 : %vMHz\n\n", int(f기준_온도()), int(f기본_클럭()), int(f현재_클럭()))
...
// f기본_클럭 : 대개의 경우 최고 클럭 대비 60% 부근 클럭에서 전력 소모 대비 최적의 성능을 나타냄.
func f기본_클럭() float64 {
	기본_클럭 := f최고_클럭() * 0.7
