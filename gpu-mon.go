...
func main() {
	...
	if 최근_온도 < f기준_온도()-5.0 && f현재_클럭() == f최저_클럭() {
...
func gpu온도_확인(최근_온도 float64) (현재_온도 float64) {
	...
	if 온도_모음, 에러 := gpu온도_측정(); 에러 == nil {
		...
		if 최저_속도_모드 && 현재_온도 < 기준_온도-10.0 && 온도_예측치 < 기준_온도-10.0 {
			f클럭_한단계_낮추기(과열_발생_클럭) // 온도가 낮아지면 클럭 정상 회복.
			최저_속도_모드 = false
		} else if !최저_속도_모드 && 온도_예측치 > 현재_온도 && 온도_예측치 > 기준_온도-5.0 {
			f클럭_한단계_낮추기(f현재_클럭()) // 온도가 상승 중인데, 기준 온도에 근접했다면 클럭 낮추기.
		} else if !최저_속도_모드 && 현재_온도 > 기준_온도-2.5 && 온도_예측치 > 기준_온도-2.5 {
			f클럭_한단계_낮추기(f현재_클럭()) // 온도 상승 무관하게 기준 온도에 아주 근접했다면 클럭 낮추기.
		}
	}
